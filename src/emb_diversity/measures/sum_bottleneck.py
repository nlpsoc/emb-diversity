...
def sum_bottleneck(
    ...
    **Range:** >= 0; grows with n unless ``normalize_by_n``; the upper bound depends on ``metric`` (e.g. <= 2n for cosine distance).
    ...
    2) For each sample, take the distance to its nearest other sample.
    3) Return the sum of those per-sample minima (or their average if
       ``normalize_by_n``).
    ...
    Args:
        ...
        normalize_by_n:
            If True, return the average per-sample minimum distance (sum / n)
    ...
    Returns:
        ...
        sum (or average if normalized) of per-sample minimum distances and
        ``parameters`` records the configuration used.

    Raises:
        ValueError: If input is invalid, empty, or has fewer than 2 datapoints.
    """
    data, embedding_model = resolve_embeddings(data, diversity_axis, embedding_model, measure="sum_bottleneck", chunking_kwargs=chunking_kwargs)
