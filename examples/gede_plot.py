...
Run it with::
    ...
    uv run python examples/gede_plot.py path/to/gede_essays.json [rewrite|task]
...
def main() -> None:
    ...
    for col, (axis_name, embed_kwargs) in enumerate(AXES):
        ...
        # ── top: PCA scatter (one frame fit on all classes for a shared view) ──
        pca = PCA(n_components=2).fit(np.vstack([emb[c] for c in PLOT_CLASSES]))
        ax = axes[0][col]
        for label in PLOT_CLASSES:
            xy = pca.transform(emb[label])
            ax.scatter(xy[:, 0], xy[:, 1], s=8, alpha=0.45,
                       c=COLORS[label], edgecolors="none", label=label)
        ax.set_title(f"{axis_name} — PCA of embeddings")
        ax.set_xticks([])
        ax.set_yticks([])
    ...
    fig.suptitle(
        ...
        f"({n_per_class} texts/class)",
        fontsize=14,
    )
    fig.tight_layout(rect=(0, 0, 1, 0.98))
    out = f"gede_{which}_semantic_vs_style.png"
